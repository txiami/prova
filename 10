...
function shuffleMusicas(musicasTocadas) {
  ...
  for(let i = 0; i<musicasTocadas.length ; i++){
      for(let j = i+1; j<musicasTocadas.length ; j++){
          if(musicasTocadas[i]>musicasTocadas[j]){
              let b = musicasTocadas[i];
              musicasTocadas[i] = musicasTocadas[j];
              musicasTocadas[j] = b;
          }
      }
  }
  
  musicasTocadas.reverse();
  for(let i = 0 ; i<musicasTocadas.length/2 ; i++){
array.push(musicasTocadas[i]);
array.push(musicasTocadas[musicasTocadas.length - (i+1)]);
  }
