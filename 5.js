...
function calculaPorcentagemSorteio(assinante,minutosAssistidos) {
    ...
    if(assinante[i]){
    horas.push(  ( Math.ceil(minutosAssistidos[i]/60) * 2 ) );
    
    }
    else{
        horas.push(Math.ceil(minutosAssistidos[i]/60));
    }
    horas[i] = Math.ceil(horas[i]);
    soma += horas[i];

        }
    
        for(let i=0 ;i<horas.length; i++){
            chances.push ( Math.round ( ( horas[i] *100) /soma ) )
        }
